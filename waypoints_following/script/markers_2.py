...
class mark():
    def __init__(self,robot_no):
        ...
        self.marker_str = "/turtlebot"
    	self.marker_str += str(self.robot_no)
    	self.marker_str += "/visualization_marker_array"
        self.frame_id_string = "robot"
    	self.frame_id_string += str(self.robot_no)
    	self.frame_id_string += "_tf/map"
        self.marker_publisher = rospy.Publisher(self.marker_str, MarkerArray, queue_size=10)
        rospy.sleep(0.5)
    ...
    def show_text_in_rviz(self, points_seq):
        ...
        while(m < n):
            ...
            marker.header = Header(frame_id=self.frame_id_string) ## frame_id='base_link' creates markers that move around with TurtleBot
        ...
        while(m < n):
            ...
            marker1.header = Header(frame_id=self.frame_id_string)
            if (self.robot_no == 1):
                marker1.color = ColorRGBA(1.0, 0.0, 0.0, 0.8)
            else:
                marker1.color = ColorRGBA(0.0, 0.0, 1.0, 0.8)
            marker1.text = '%d'%m
    ...
    def clear_text_in_rviz(self, points_seq):
        ...
        while(m < n):
            ...
            marker.pose = Pose(Point(points_seq[m][0], points_seq[m][1], 0.0), Quaternion(0, 0, 0, 1))
            marker.scale = Vector3(0.06, 0.06, 0.06)
            marker.header = Header(frame_id=self.frame_id_string)
            marker.color = ColorRGBA(0.0, 1.0, 0.0, 0.8)
            markerArray.markers.append(marker)
            m += 1
        m = 0
